...
Use of `goal` package and more specificaly the 'osm' group of functions for the analysis of [OpenStreetMap](https://www.openstreetmap.org) POIs (Points of Interest) and road network. 
...
Alternatively, we can use the bounding box of an area instead of the name of a municipality.
```{r}
myt = osm.getPOI_usingbb( c(26.547303,39.101658,26.564641,39.113247), inkey ="amenity" )
myt
```

After downloading the POIs (`am`), we merge all POIs together (Points, Polygons, Multipolygons), and convert them to points.
```{r}
